 ...
 * @date        $Date: 2012-12-03 11:15:02 +0100 (lun., 03 déc. 2012) $:
 */
...
class Servers extends HTTPClient
{
     ...
     * @throws Exception
     */
    public function getOnlinePlayers($login)
    {
        if (!$login) {
            throw new Exception('Invalid login');
        }
     ...
     * @throws Exception
     */
    public function getFavoritedCount($login)
    {
        if (!$login) {
            throw new Exception('Invalid login');
        }
     ...
     * @throws Exception
     */
    public function getReportAbuses($login)
    {
        if (!$login) {
            throw new Exception('Invalid login');
        }
